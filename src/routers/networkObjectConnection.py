...
class networkObjectAPI(BaseAPI):
    ...
    def get_all_networkObjectConnections(self, current_user: DBUser = Depends(get_current_user)):
        # KI Claude <KI-4>
        ...
        return self.db.query(models.DBNetworkObjectConnection).filter(
            models.DBNetworkObjectConnection.id.in_(visible_conn_ids)
        ).all()
        # KI END <KI-4>
